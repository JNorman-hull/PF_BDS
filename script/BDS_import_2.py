...
class RAP_HIG(Rapid):
    ...
    def _post_process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data["accmag"] -= 9.81
        data = data[["time", "accx", "accy", "accz", "accmag"]]
        return data
...
#Process BDS250 files
    ...
    rapid_hig_files = list(Path("./RAW_data/RAPID").glob("*.HIG"))
    print(f"Processing {len(rapid_hig_files)} RAPID HIG files...")
    for f in rapid_hig_files:    
        filename = f
        print(f"Processing {filename.name}")
        mymeas = BDS100(filename)
        _, summary_info = mymeas._process_and_save(True)
...
#Create summary CSV
    ...
    print(f"{len(summary_data)}/{len(bds100_files) + len(bds250_files) + len(rapid_hig_files)}  txt files processed")
